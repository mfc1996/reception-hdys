...
public class AlipayController {
    ...
    // 获取配置文件的信息
    String app_id = AlipayConfig.app_id;//appid
    String private_key = AlipayConfig.merchant_private_key;//私钥
    String notify_url = AlipayConfig.notify_url;//服务器异步通知页面路径
    String return_url = AlipayConfig.return_url;//页面跳转同步通知页面路径
    String url = AlipayConfig.gatewayUrl;//支付宝网关
    String charset = AlipayConfig.charset;//字符编码
    String format = AlipayConfig.format;
    String public_key = AlipayConfig.alipay_public_key;//公钥
    String signtype = AlipayConfig.sign_type;//签名方式
    ...
    @RequestMapping("/pay")
    public void pay(HttpServletRequest request, HttpServletResponse response,String money,Integer goodsId) throws Exception {

        // 模拟从前台传来的数据
        /*String orderNo =(String)((Math.random()*9+1)*1000000000);*/
        int code = (int) Math.ceil(Math.random()*899999+100000); // 生成订单号// 生成订单号
        String orderNo = String.valueOf(code);
        /*homeServiceFeign.queryGoodsId(goodsId,orderNo);*/
        System.out.println("生成订单号"+orderNo);
        String totalAmount = money; // 支付总金额
        String subject = "ChenK"; // 订单名称
        System.out.println("订单名称"+subject);
        String body = "ChenK"; // 商品描述
        // 封装请求客户端
